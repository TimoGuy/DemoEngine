...
std::vector<float> ShaderExtCSM_shadow::cascadeShadowMapTexelSizes;
...
void ShaderExtCSM_shadow::setupExtension()
{
	...
	for (size_t i = 0; i < glm::min((size_t)16, cascadePlaneDistances.size()); i++)
	{
		...
		shader->setFloat("cascadeShadowMapTexelSizes[" + std::to_string(i) + "]", cascadeShadowMapTexelSizes[i]);
	}
