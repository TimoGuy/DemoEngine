...
void main()
{
    ...
    vec3 F0 = vec3(0.04);       // NOTE: this marks the material as dia-electric, but this is for a clear-coat-like look
    ...
    for(int i = 0; i < numLights; ++i)      // NOTE: Change this depending on the number of lights being looped over
    {
        ...
        Lo += (kD * zellyColor / PI + specular) * radiance * NdotL * (1.0 - shadow);  // note that we already multiplied the BRDF by the Fresnel (kS) so we won't multiply by kS again
    }   
    ...
    vec3 diffuse = irradiance * zellyColor;
